...
class Mdns extends EventEmitter2 {
  ...
  start () {
    ...
    if (!_.isNull(this.browser)) {
      this.logger.debug({ name: 'mdns', msg: 'start' })
      this.browser
        .on('ready', function () {
          if (_this.timeout > 0) {
        ...
        .on('update', function (srv) {
          _this.logger.debug({ name: 'mdns', msg: 'service.found', properties: srv })
  ...
  stop () {
    this.logger.debug({ name: 'mdns', msg: 'stop' })
    if (this.browser) {
      /**
       ...
       * @type {Object}
       */
      this.emit('done', this.foundServices)
      this.browser.stop()
